...
from common_utils import CUDA_NOT_AVAILABLE_MSG, IN_FBCODE, IN_OSS_CI, IN_RE_WORKER, OSS_CI_GPU_NO_CUDA_MSG, IPU_NOT_AVAILABLE_MSG


def pytest_configure(config):
    # register an additional marker (see pytest_collection_modifyitems)
    config.addinivalue_line("markers", "needs_cuda: mark for tests that rely on a CUDA device")
    config.addinivalue_line("markers", "needs_ipu: mark for tests that rely on a IPU device")
...
def pytest_collection_modifyitems(items):
    ...
    for item in items:
        ...
        needs_ipu = item.get_closest_marker("needs_ipu") is not None
        ...
        if needs_ipu:
            ...
            if torch_backend:
                ...
                item.add_marker(pytest.mark.skip(reason=IPU_NOT_AVAILABLE_MSG))
